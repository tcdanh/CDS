...
class ResearchInfoController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        if ($canManageProfiles && ! $request->filled('user')) {
            $personalInfos = PersonalInfo::with(['user:id,name'])
                ->withCount(['researchProjectRecords'])
        ...
        $info->load([
            'researchProjectRecords' => fn ($query) => $query->orderBy('position')->orderBy('id'),
        ]);

        return view('scientific_profiles.research', [
            'info' => $info,
    ...
    public function edit(Request $request): View
    {
        ...
        return view('scientific_profiles.edit_research', [
            'info' => $info,
        ]);
    }
}
